...
#    global enfuse_bin
    ...
    parser.add_argument("method", type=str, help="Method of averaging: 'convert' : ImageMagick -average, 'enfuse' : Hugin\'s enfuse, 'pil', python Pillow library.")


    parser.add_argument("num", type=int, help='Number of images to combine per frame.')

    parser.add_argument("-b", "--skip-frames", type=int, default=1, help='Number of images to skip. Ususally for doing HDR set this to the number of images in each set.')

    parser.add_argument('--shuffle-frames', nargs='?', const=1000000, type=int, default=0,
                        help='shuffle frames by XXX frames (default = 1000000) Large default to get a completely random shuffle.')

    parser.add_argument('-m', '--hard-mask',  action='store_true', default=False,
                        dest='hardmask',
...
#    print(args)
    ...
    process_dir(args.num, args.hardmask, args.shuffle_frames, args.skip_frames,
                    args.method, args.input_dir, args.output_dir)
    print("DONE.")
